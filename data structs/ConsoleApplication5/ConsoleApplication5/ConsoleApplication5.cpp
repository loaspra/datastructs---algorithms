...
public:
    //Constructor
    Diccionario_open(E a, int tamanho = 100);
...
public:
    ...
    int prof = 0;
    void rutas_aux(int pos, int lim, ListaArreglo<string> ss)
    {
        if (prof > lim)
            return;
        marca[pos] = 1;
        cout << ss.getValor(pos) << ", ";
    ...
    void rutas(int it, int pos, ListaArreglo<string> ss)
    {
        rutas_aux(it, pos, ss);
    }
...
int main()
{
    ...
    ListaArreglo<string> v1;
    ListaArreglo<string> v2;
    Diccionario_open<string, int>* dic = new Diccionario_open<string, int>(NULL);
    string a, b;
    int en;
    cin >> en;
    for (int i = 0; i < en; i++)
    ...
    int size = sett.longitud();
    string s;
    int ite = 0;
    while (sett.longitud() > 0)
    {
        s = sett.removerCualquiera();
        ss.agregar(s);
        cout << s << "?" << ss.getValor(ite) << endl;
        dic->insertar(s, ite);
        ite++;
    }

    GraphM* equis = new GraphM(size);
    int c = 0;
    string persona;
    for (ss.moverAInicio(); ss.posicionActual() < ss.longitud(); ss.siguiente())
    {
        equis->setArista(dic->buscar(v1.getValor()), dic->buscar(v2.getValor()), 1);
        c++;
    }
    int profundidad;
    cin >> persona;
    cin >> profundidad;
    equis->rutas(dic->buscar(persona), profundidad, ss);
...
//Constructor
template<typename Key, typename E>
inline Diccionario_open<Key, E>::Diccionario_open(E a, int tamanho)
{
    lista = new E[tamanho];
    this->valor_nulo = a;
